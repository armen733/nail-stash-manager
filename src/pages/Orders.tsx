  ...
  const fetchData = async () => {
    try {
      const [ordersRes, salonsRes, productsRes, profilesRes] = await Promise.all([
        supabase.from("orders").select("*, salons(name), order_items(id, quantity, unit_price, product_id, products(name, sku, image_url, product_images(image_url)))").order("order_date", { ascending: true }).order("created_at", { ascending: true }),
